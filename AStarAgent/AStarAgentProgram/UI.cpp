...
//Specifically prompts user twice once for inital state another for goal state
void acceptInitalAndGoalInputs(int(&init)[yAxis][xAxis], int(&goal)[yAxis][xAxis]) {
	acceptConfigInput(init);
	acceptConfigInput(goal);
}
